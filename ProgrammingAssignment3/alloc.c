...
struct Pool
{
	...
	int	nbrk;
	int	lastfree;
	Bhdr* slabs[8]; // For slab allocation
	void	(*move)(void*, void*);
};
...
void  
printSlabList(Pool *p) {
        int i;
        Bhdr* s;

	print("Looking at Pool %s slab\n", p->name);
        for (i=0; i<8; i++) {
                s = p->slabs[i];
                if (s == nil) {
                        print("Slab list %d is empty\n", i);
                } else {
                        print("Slab list %d: ", i);
                        while(s != nil) {
                                print("[addr:%p, magic:%ux], ", s, s->magic);
                                s = s->fwd;
                        }
                }
                print("\n");
...
dopoolalloc(Pool *p, ulong asize, ulong pc)
{
	...
	lock(&p->l);
	// Checking if request size < 4K, if it is, round it up!
        if (size <= 4096) {
                size = slabRound(size);
		if (size == 32) {
			idx = 0;
		...
		//print("\nChecking our slab free list before allocating %d bytes\n", size);
		//printSlabList(p);

		// Check the pool's slab list
		if (p->slabs[idx] != nil) {
			//print("Allocating %d bytes from slab memory\n", size);
			// Grab the slab at the head of the list
			t = p->slabs[idx];
...
poolfree(Pool *p, void *v)
{
	...
	// Check if block can go in slab list
	if (b->size <= 4096) {	

               	//print("\nAttempting to add slab block of size %ud\n", b->size);
		//printSlabList(p);
		...
		if (idx != -1) {
			...
			if (p->slabs[idx] == nil) {
				...
				/* Add to head of slab list */
				c = p->slabs[idx];
				p->slabs[idx] = b;
				p->slabs[idx]->fwd = c;
			
			}
			//print("\nChecking slab list after adding slab block of size %ud\n", b->size);
			//printSlabList(p);
			unlock(&p->l);
			return; 
		} 
