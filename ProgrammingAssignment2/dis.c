...
// Kasper - Get highest the highest priority on the process ready list
int
getHighestPriority(void)
{
	Prog *p;

	int highPriority = 1;

	for (p = isched.runhd; p != nil; p = p->link) {
...
addrun(Prog *p)
{
	...
	// Kasper - checks for me
	/*if (p->state == Psend) {
		print("Previous state of %d was Psend\n",p->pid);
	}
	if (p->state == Precv) {
                print("Previous state of %d was Prevc\n",p->pid);
        }
	if (p->state == Palt) {
                print("Previous state of %d was Palt\n",p->pid);
        }*/

	// Change quanta
	if ((p->state == Psend) || (p->state == Precv) || (p->state == Palt)) {
		p->priority++;
        	if ( p->priority >= 3) {
                	p->priority = 3;
        	}		
        	p->quanta = PQUANTA + (1000 * (p->priority - 1));
	}
	...
	for(;;) {
		...
		if(r != nil) {
			...
			if(isched.runhd != isched.runtl) {
				...
				// Kasper - Priority scheduling 
				highPriority = getHighestPriority();
				//print("highPriority = %d\n",highPriority);
				while(isched.runhd->priority != highPriority) {
					r = isched.runhd;
					isched.runhd = r->link;
                                	r->link = nil;
                                	isched.runtl->link = r;
				...
				//print("Process %d with priority %d is getting scheduled next\n",isched.runhd->pid,isched.runhd->priority);
...
disinit(void *a)
{
	...
	// Printing host name and kernel build date
	print("Host name: \"%s\"\n",eve);
	print("Kernel Build Date: %lud (in epoch)\n", kerndate);
